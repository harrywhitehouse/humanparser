...
parser.parseName = function (name) {
  ...
  var suffixes = ['i', 'ii', 'iii', 'iv', 'v', 'senior', 'junior', 'jr', 'sr', 'phd', 'apr', 'rph', 'pe', 'md', 'ma', 'dmd', 'cme', 'qc', 'kc'];
  var compound = ['vere', 'von', 'van', 'de', 'del', 'della', 'der', 'di', 'da', 'pietro', 'vanden', 'du', 'st.', 'st', 'la', 'lo', 'ter', 'bin', 'ibn', 'te', 'ten', 'op', 'ben', 'al'];
  var parts = name.trim().replace(/\b\s+(,\s+)\b/, '$1') // fix name , suffix -> name, suffix
  ...
  var firstNameFirstFormat = parts.every(function (part) {
  ...
  if (!firstNameFirstFormat) {
    ...
    var firstNameIndex; //location of first comma will separate last name from rest
    //join all parts leading to first comma as last name

    var lastName = parts.reduce(function (lastName, current, index) {
      ...
      if (current.indexOf(',') === -1) {
        ...
        current = current.replace(',', ''); // handle case where suffix is included in part of last name (ie: 'Hearst Jr., Willian Randolph')
    ...
    if (parts.length > 1 && salutations.indexOf(parts[0].toLowerCase().replace(/\./g, '')) > -1) {
      attrs.salutation = parts.shift(); // if we have a salutation assume 2nd part is last name
...
parser.getFullestName = function (str) {
  var name = str;
  var names = []; //find fullname from strings like 'Jon and Sue Doyle'

  if (name.indexOf('&') > -1 || name.toLowerCase().indexOf(' and ') > -1) {
    names = name.split(/\s+(?:and|&)\s+/gi); //pluck the name with the most parts (first, middle, last) from the array.
...
parser.parseAddress = function (str) {
  str = str.replace(/\n/gi, ', '); //416 W. Manchester Blvd., Inglewood, CA  90301
